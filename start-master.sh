...
start_jupyterlab() {
    ...
    cat > /opt/miniconda3/etc/conda/activate.d/bigdata-env.sh << 'EOF'
#!/bin/bash
# 设置 Hadoop 环境变量
...
EOF
    ...
    # 创建 deactivate 脚本（可选）
    mkdir -p /opt/miniconda3/etc/conda/deactivate.d
    cat > /opt/miniconda3/etc/conda/deactivate.d/bigdata-env.sh << 'EOF'
#!/bin/bash
# 清理环境变量（可选）
...
EOF
    ...
    chmod +x /opt/miniconda3/etc/conda/deactivate.d/bigdata-env.sh
...
EOF
    ...
    if kill -0 $JUPYTER_PID 2>/dev/null; then
        ...
        echo "查看 JupyterLab token: /opt/miniconda3/bin/jupyter lab list"
        echo ""
        ...
        echo "  source /opt/miniconda3/etc/conda/activate.d/bigdata-env.sh"
    else
